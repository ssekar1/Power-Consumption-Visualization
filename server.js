...
var SampleApp = function() {
	...
	self.routes['/js/histogram'] = function(req, res)
	{
		res.setHeader('Content-Type', 'application/js');
	...
	{
		res.setHeader('Content-Type', 'application/js');
		res.send(self.cache_get('powerGraph.js'));
	};
	...
	self.routes['/data/raw/:circuits/:start/:end'] = function(req, res) {
		...
		queryString += " FROM powerreadings WHERE ? <= unixTimestamp AND unixTimestamp <= ? order by unixTimestamp";
